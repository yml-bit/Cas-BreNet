...
class Generator(object):
    ...
    def __call__(self, sample):
        ...
        # img_patche = img_patche * label_patche
        # img_patche = torch.cat((img_patche, label_patche), dim=1)
...
class RandomGenerator1(object):
    ...
    def __call__(self, sample):
        ...
        # if (z_max-z_min)>self.output_size[0]:
        #     crop_z_down = z_min +random.randint(-4, 4)
        #     crop_z_up = z_max +random.randint(-4, 4)
        a=32
        b=a//2
        if (y_max - y_min) > self.output_size[1]:
            c=(y_max - y_min-self.output_size[1])//a+1
            crop_y_down = crop_y_down-b*c
            crop_y_up = crop_y_up+b*c
        if (x_max - x_min) > self.output_size[2]:
            c = (x_max - x_min - self.output_size[2]) // a + 1
        ...
        if self.mode == 'train':
            ...
            if random.random() > 0.5:
                ...
                image += np.random.normal(loc=0, scale=sigma, size=image.shape)
...
class Generator1(object):
    ...
    def __call__(self, sample):
        image, label = sample['image'], sample['label']
        # label=remove_regions(label)
        # labeled_mask, num_features = llabel(label1)  # 假设label是正确调用的函数，如果不是请替换为相应操作
        # volumes = np.bincount(labeled_mask.ravel(), weights=np.ones_like(labeled_mask).ravel().astype(int))  # 修改这里，将llabel替换为labeled_mask，并确保权重也为整型
        # largest_label = np.argmax(volumes[1:]) + 1  # 跳过背景（值为0）
        # label = labeled_mask == largest_label
        ...
        # if (z_max-z_min)>self.output_size[0]:
        #     patch_start[0] = z_min +random.randint(-4, 4)
        #     patch_end[0] = z_max +random.randint(-4, 4)
        a=32
        b=a//2
        if (y_max - y_min) > self.output_size[1]:
            c=(y_max - y_min-self.output_size[1])//a+1
            patch_start[1] = patch_start[1]-b*c
            patch_end[1] = patch_end[1]+b*c
        if (x_max - x_min) > self.output_size[2]:
            c = (x_max - x_min - self.output_size[2]) // a + 1
            patch_start[2] = patch_start[2]-b*c
            patch_end[2] = patch_end[2]+b*c
        img_patche = image[patch_start[0]:patch_end[0],
                     patch_start[1]:patch_end[1],
                     patch_start[2]:patch_end[2]]
