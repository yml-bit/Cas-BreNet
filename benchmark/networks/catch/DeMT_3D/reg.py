...
# nuf = {'A': [64, 32, 1, 64, 64, 64, 32], }

# ndf = {'A': [32, 64, 64, 64, 64], } #原始
# nuf = {'A': [64, 64, 64, 64, 32], }

ndf = {'A': [32, 64,64, 64, 64, 64], }
nuf = {'A': [64, 64,64, 64, 64, 32], }

# ndf = {'A': [24, 48, 48, 48, 48], }
# nuf = {'A': [48, 48, 48, 48, 24], }

#2.5D
# ndf = {'A': [32, 64, 64, 64, 16, 32, 32], }
# nuf = {'A': [32, 32, 1, 64, 64, 64, 32], }
...
class ResUnet3D(torch.nn.Module):
    def __init__(self, nc_a, nc_b, cfg, init_func, init_to_identity):
        ...
        for out_nf in ndf[cfg]:
            setattr(self, 'down_{}'.format(conv_num),
                    DownBlock3D(in_nf, out_nf, 3, 1, 1, activation=act, init_func=init_func, bias=True,
        ...
        if use_down_resblocks[cfg]:
            self.c1 = Conv3D(in_nf, 2 * in_nf, 1, 1, 0, activation=act, init_func=init_func, bias=True,
                           use_resnet=False, use_norm=False)
            self.t = ((lambda x: x) if resnet_nblocks[cfg] == 0
                      else ResnetTransformer3D(2 * in_nf, resnet_nblocks[cfg], init_func))
            self.c2 = Conv3D(2 * in_nf, in_nf, 1, 1, 0, activation=act, init_func=init_func, bias=True,
                           use_resnet=False, use_norm=False)
        # ------------- Up-sampling path
        act = up_activation[cfg]
        for out_nf in nuf[cfg]:
            setattr(self, 'up_{}'.format(conv_num),
                    Conv3D(in_nf + skip_nf['down_{}'.format(conv_num)], out_nf, 3, 1, 1, bias=True, activation=act,
                         init_fun=init_func, use_norm=False, use_resnet=False))
            in_nf = out_nf
            conv_num -= 1
        if refine_output[cfg]:
            self.refine = nn.Sequential(ResnetTransformer3D(in_nf, 1, init_func),
                                        Conv3D(in_nf, in_nf, 1, 1, 0, use_resnet=False, init_func=init_func,
                                             activation=act,
                                             use_norm=False)
                                        )
        else:
            self.refine = lambda x: x
        self.output = Conv3D(in_nf, 3, 3, 1, 1, use_resnet=False, bias=True,
    ...
    def forward(self, img_a, img_b):
        ...
        while conv_num > (self.ndown_blocks - self.nup_blocks):
            ...
            x = F.interpolate(x, (s.size(2), s.size(3), s.size(4)), mode='trilinear')##
            x = torch.cat([x, s], 1)
...
class Reg3D(nn.Module):
    def __init__(self,in_channels_a,in_channels_b):
       ...
       #height,width=256,256
        ...
        self.offset_map = ResUnet3D(self.in_channels_a, self.in_channels_b, cfg='A', init_func=init_func, init_to_identity=init_to_identity).to(
            self.device)
