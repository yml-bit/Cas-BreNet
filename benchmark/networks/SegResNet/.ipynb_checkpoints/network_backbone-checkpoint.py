...
class SegResNet(nn.Module):
    ...
    def __init__(
        ...
        self.up_layers2, self.up_samples2 = self._make_up_layers()
        self.conv_final = self._make_final_conv(out_channels)
        self.predict_type=self._classifier(class_num=12)
    ...
    def _classifier(self, class_num: int):
        in_channel=self.init_filters*2**(self.blocks_down[-1]-1)
        return nn.Sequential(nn.AdaptiveAvgPool3d((1, 1, 1)),
                             nn.Flatten(), nn.Linear(in_channel, class_num))

    def forward(self, x: torch.Tensor) -> tuple[Tensor, Sequential]:
        x, down_x = self.encode(x)
        type = self.predict_type(x)
        down_x.reverse()
