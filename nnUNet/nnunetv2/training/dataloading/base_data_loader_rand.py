...
class nnUNetDataLoaderBase(DataLoader):
    def __init__(self,
                 ...
                 ##### 更改 oversample_foreground_percent 参数
                 oversample_foreground_percent: Callable[[], float] = lambda: np.random.uniform(0.11, 0.44),
    ...
    ####{ 修改这里对oversample_foreground_percent的引用
        self.get_do_oversample_probability = oversample_foreground_percent if probabilistic_oversampling \
            else self._oversample_last_XX_percent

    def _probabilistic_oversampling(self, sample_idx: int) -> bool:
        # 使用新的 oversample_foreground_percent 方法获取概率
        oversample_prob = self.get_do_oversample_probability()
        return np.random.uniform() < oversample_prob
    #######}
