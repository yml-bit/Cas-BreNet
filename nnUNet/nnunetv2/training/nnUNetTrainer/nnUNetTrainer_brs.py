...
from .Mednext.MedNextV1 import MedNeXt as MedNeXt#增加了输出层
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.num_epochs = 500##1000  400x250=10 0000
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.loss, self.loss2= self._build_loss()
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        data = batch['data']
        target = batch['target']
        # binary_mask = (copy.deepcopy(target[0]) != 0).type(torch.uint8)
        # data = torch.cat((data, binary_mask, data* binary_mask), dim=1)
        # 遍历列表中的每一个张量并将其大于0的元素置为1
