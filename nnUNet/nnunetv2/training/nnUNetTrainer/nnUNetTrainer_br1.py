...
from .Mednext.MedNext_b import MedNeXt as MedNeXt#增加了输出层
from monai.transforms import AsDiscrete
from nnunetv2.training.loss.loss.losses import SSIMLoss
from monai.losses import DiceFocalLoss,TverskyLoss,GeneralizedDiceLoss
import copy
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.oversample_foreground_percent = 0.99 # 0.33
        self.num_iterations_per_epoch = 250
        self.num_val_iterations_per_epoch = 50 #self.save_every = 10
        self.num_epochs = 30##1000  400x250=10 0000
        self.current_epoch = 0
        self.enable_deep_supervision = True #True False
        self.to_label = AsDiscrete(argmax=True, to_onehot=2)

        self.TverskyLoss = TverskyLoss(to_onehot_y=True,softmax=True,reduction="mean")
        ...
        self.loss1 = None  # -> self.initialize
        self.loss2 = None  # -> self.initialize
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.loss1,self.loss2= self._build_loss()
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        # model = MedNeXt(in_channels=3,n_channels=32,
        #     n_classes=3,exp_r=[2,3,4,4,4,4,4,3,2],
        #     kernel_size=3,deep_supervision=True,do_res=True,#step1-3;step2-5
        #     do_res_up_down=True,block_counts=[3,4,4,4,4,4,4,4,3],#True False
        #     checkpoint_style='outside_block')

        # model = MedNeXt(in_channels=3,n_channels=32,
        #     n_classes=3,exp_r=[3, 4, 8, 8, 8, 8, 8, 4, 3],
        #     kernel_size=3,deep_supervision=True,do_res=True,
        #     do_res_up_down=True,block_counts=[3, 4, 8, 8, 8, 8, 8, 4, 3],#True False
        #     checkpoint_style='outside_block')
        model = MedNeXt(in_channels=3,n_channels=32,
            n_classes=3,exp_r=[2,3,4,4,4,4,4,3,2],
            kernel_size=3,deep_supervision=True,do_res=True,#step1-3;step2-5
    ...
    def _build_loss(self):
        ...
        # loss2=DiceFocalLoss(to_onehot_y=True, softmax=True)
        # loss1=nn.CrossEntropyLoss()#step-1

        w = torch.tensor([1.0, 1.0, 3.0]).to(self.device)#step-2
        loss1 = nn.CrossEntropyLoss(weight=w)

        loss2=FocalLoss(alpha=0.25, gamma=2)
        if self.enable_deep_supervision:
            ...
            # now wrap the loss
            loss1 = DeepSupervisionWrapper(loss1, weights)
            loss2=DeepSupervisionWrapper(loss2, weights)
        return loss1,loss2
    ...
    def configure_optimizers(self):
        ...
        # optimizer = AdamW(self.network.parameters(),lr=self.initial_lr,weight_decay=self.weight_decay,
        #                   amsgrad=True)
    ...
    def train_step(self, batch: dict) -> dict:
        data = batch['data']
        aa = batch['target']
        binary_mask = (copy.deepcopy(aa[0]) != 0).type(torch.uint8)
        data = torch.cat((data, binary_mask, data* binary_mask), dim=1)
        target=[]
        for t in aa:
            max_values= torch.amax(t, dim=(1, 2, 3, 4))
            target.append(torch.Tensor(max_values).long())
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # output = self.to_label(output)#transform one_hot to
            # del data
            # print(output[0].max())
            l = self.loss1(output, target)
            # l = self.loss2(output, target)
            # l = self.loss1(output, target) + self.loss2(output, target)
    ...
    def on_train_epoch_end(self, train_outputs: List[dict]):
        ...
        if self.is_ddp:
            ...
            weight_loss =outputs['loss']####
            loss_here = np.mean(weight_loss)
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        # global_dc_per_class = [i for i in [(i+m) / (i + j + k+m) for i, j, k,m in zip(tp, fp, fn,tn)]]#acc  良恶性用该函数。整体

        sen=tp[0]/(tp[0]+fn[0])
        spe=tn[0]/(tn[0]+fp[0])
        global_dc_per_class=[sen,spe]
        global_dc_per_class = np.nan_to_num(global_dc_per_class, nan=0.01)


        # precision = [i for i in [(i) / (i + j ) for i, j in zip(tp, fp)]]
        # recall = [i for i in [(i) / (i +  k ) for i,  k in zip(tp, fn)]]
        # global_dc_per_class = [i for i in [(i*k) / (i + k ) for i,  k in zip(precision, recall)]]#
        # global_dc_per_class = np.nan_to_num(global_dc_per_class, nan=0.01)
        # weight=np.array([0.66, 1.3])#### [1, 1, 2, 5] [1, 1, 3]
        # mean_fg_dice = np.nanmean(global_dc_per_class*weight)  # weight soft plaque

        mean_fg_dice = np.nanmean(global_dc_per_class)
        self.logger.log('mean_fg_dice', mean_fg_dice, self.current_epoch)
        # self.logger.log('weight_mean_fg_dice', weight_mean_fg_dice, self.current_epoch)
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        if not self.was_initialized:
            self.initialize()
        # loadpath=filename_or_checkpoint.replace("checkpoint_final","checkpoint_best")
