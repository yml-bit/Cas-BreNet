...
class nnUNetTrainer(object):
    ...
    def _build_loss(self):
        ...
        # w=torch.tensor([1.0,1.0, 1.0,3.0]).to(self.device)
        # loss2 = nn.CrossEntropyLoss(weight=w)
        # loss2 = nn.CrossEntropyLoss()
        if self.enable_deep_supervision:
    ...
    def train_step(self, batch: dict) -> dict:
        data = batch['data']
        aa = batch['target']
        # binary_mask = (copy.deepcopy(aa) != 0).type(torch.uint8)
        # data = torch.cat((data, binary_mask, data* binary_mask), dim=1)
        # max_values = torch.amax(aa, dim=(1, 2, 3, 4))
        # target=torch.Tensor(max_values).long()
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            # a=0.5
            # l = self.loss(output, target)*0.5 + self.loss2(output, target)*0.5
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        global_dc_per_class = [i for i in [(i+m) / (i + j + k+m) for i, j, k,m in zip(tp, fp, fn,tn)]]
        mean_fg_dice = np.nanmean(global_dc_per_class)#weight soft plaque

        # weight=np.array([1, 1, 1.25, 1.5])#### [1, 1, 2, 5] [1, 1, 3]
