...
##探索阶段使用
class LesionClassifier0(nn.Module):
    def __init__(self, in_channels, num_classes=6):
        super(LesionClassifier0, self).__init__()
...
class LesionClassifier2(nn.Module):
    def __init__(self, input_nc, num_classes):
        super(LesionClassifier2, self).__init__()
...
class MedNeXt(nn.Module):
    ...
    def __init__(self, 
        ...
        self.out_0 = LesionClassifier(n_channels, n_classes)

        # Used to fix PyTorch checkpointing bug
        self.dummy_tensor = nn.Parameter(torch.tensor([1.]), requires_grad=True)  

        if deep_supervision:
            self.out_1 = LesionClassifier(n_channels * 2, n_classes)
            self.out_2 = LesionClassifier(n_channels * 4, n_classes)
            self.out_3 = LesionClassifier(n_channels * 8, n_classes)
            self.out_4 = LesionClassifier(n_channels * 16, n_classes)
    ...
    def forward(self, x):
        ...
        if self.outside_block_checkpointing:
            ...
            if self.do_ds:
                x_ds_4 = checkpoint.checkpoint(self.out_4, x, use_reentrant=False)

            x_up_3 = checkpoint.checkpoint(self.up_3, x, self.dummy_tensor)
            dec_x = x_res_3 + x_up_3
            x = self.iterative_checkpoint(self.dec_block_3, dec_x)
            if self.do_ds:
                x_ds_3 = checkpoint.checkpoint(self.out_3, x, use_reentrant=False)
            del x_res_3, x_up_3
            ...
            if self.do_ds:
                x_ds_2 = checkpoint.checkpoint(self.out_2, x, use_reentrant=False)
            del x_res_2, x_up_2
            ...
            if self.do_ds:
                x_ds_1 = checkpoint.checkpoint(self.out_1, x, use_reentrant=False)
            del x_res_1, x_up_1
            ...
            x = checkpoint.checkpoint(self.out_0, x, use_reentrant=False)
            ...
            # class_output = self.classification_head(xx)
            x = self.out_0(x)
