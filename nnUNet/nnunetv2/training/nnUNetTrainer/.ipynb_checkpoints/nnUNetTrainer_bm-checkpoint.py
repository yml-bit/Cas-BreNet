...
# from .Mednext.MedNextV1 import MedNeXt#原始
from .Mednext.MedNextV2 import MedNeXt as MedNeXt#增加了输出层
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.num_epochs = 200##1000  400x250=10 0000
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        # model = MedNeXt(in_channels=2,n_channels=32,
        #     n_classes=5,exp_r=[2,3,4,4,4,4,4,3,2],
        #     kernel_size=5,deep_supervision=False,do_res=True,
    ...
    def _build_loss(self):
        ...
        # loss2=GeneralizedDiceLoss(to_onehot_y=True, softmax=True)
        if self.enable_deep_supervision:
    ...
    def train_step(self, batch: dict) -> dict:
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            output, output2= self.network(data)
            # output = self.to_label(output)#transform one_hot to
            # del data
            # print(output[0].max())
            # print(target[0].size())
            flat_target = target[0].reshape(target[0].shape[0], -1)
            z=flat_target.max(dim=1).values.long()
            # print(output2.size())
            # print(z.size())
            l = self.loss(output, target)+self.loss_class(output2, z)
