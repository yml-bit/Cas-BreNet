...
def read_predictions(file_path):
    ...
    with open(file_path, 'r') as file:
        for line in file:
            id, pred, true,prob = line.strip().split()
            id = str(id)
            pred = int(pred)
            true = int(true)
            prob=float(prob)
            if id not in predictions:
                predictions[id] = {'preds': [], 'true': true,'prob':[]}
            predictions[id]['preds'].append(pred)
            predictions[id]['prob'].append(prob)
    return predictions
...
def majority_voting(predictions):
    ...
    for id, data in predictions.items():
        ...
        voted_pred = max(pred_counts, key=lambda k: pred_counts[k])
        max_value = pred_counts[voted_pred]
        indices = [index for index, value in enumerate(data['preds']) if value == voted_pred]
        # indices = np.array(indices)
        # pp = data['prob']
        pp = [data['prob'][i] for i in indices]
        mean_p=np.mean(pp)
        std=np.std(pp)
        voted_preds.append((id, voted_pred, data['true'],std,max_value))
    return voted_preds
    ...
    for file_path in file_paths:
        ...
        for id, data in predictions.items():
            if id not in all_predictions:
                all_predictions[id] = {'preds': [], 'true': data['true'],'prob':[]}
            all_predictions[id]['preds'].extend(data['preds'])
            all_predictions[id]['prob'].extend(data['prob'])
...
if __name__ == '__main__':
    path_list = []
    #set1_step1
    for root, dirs, files in os.walk("./set1_step22/", topdown=False):#set1_step1 set1_step2
        for file in files:
            path = os.path.join(root, file)
            if "external" in path:
                path_list.append(path)
    voted_results = process_files(path_list)
    file = "./set1_step22/total14.txt" #注意剔除第一步就将病变漏诊的数据
    f_test = open(file, "w")
    y_true=[]
    y_pred=[]
