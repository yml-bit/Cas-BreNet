...
if __name__ == '__main__':
    ...
    src_data_folder = os.path.join(nnUNet_raw, "external/")  # set1 external
    path_list = []
    ...
    val = "../dataset_conversion/breast_list/val" + str(0) + ".txt"
    ff = open(val)  # train
    val_list = []  # train
    ...
    file = "./record_set1_a.txt"  # set1  external a b c d e
    f_test = open(file, "w")
    ij = 0
    for path in path_list:  #
        label_path = path.replace("0.nii.gz", "22.nii.gz")
        ID = path.split("/")[-2]
        ...
        # ret=remove_regions(ret)#直接计算最终的结果
        label = sitk.GetImageFromArray(ret.astype(np.int16))
        #unet MedNeXtl MedNeXtx1c MedNeXtx1
        out_put=label_path.replace("external","external_test")#lo:97  x1:54  xlo2:10
        # out_put = label_path.replace("set1", "set1_test")  # lo:97  x1:54  xlo2:10
