...
class nnUNetPredictor(object):
    ...
    def predict_single_npy_array(self, input_image: np.ndarray, image_properties: dict,label:np.ndarray,
        ...
        if output_file_truncated is not None:
            ...
            ret1, ret2 = torch.max(predicted_logits.data, dim=1)#1
    ...
    def _internal_predict_sliding_window_return_logits(self,
        ...
        for sl in tqdm(slicers, disable=not self.allow_tqdm):
            ...
            if not predicted_logits:
                ...
                sorted_tensor, indices = torch.sort(prediction, descending=True)
                if sorted_tensor[0,0] < 0 or (sorted_tensor[0,0]-sorted_tensor[0,1])<0.1:
                    continue
                else:
                    predicted_logits.append(prediction)  #
...
if __name__ == '__main__':
    ...
    file = "./set1_step22/record_external_80e.txt" #set1  external a b c d e
    ...
    for path in path_list:#
        ...
        ret1,ret2 = predictor.predict_single_npy_array(img, props, label,None, None, False)
        y_pred.append(int(ret2.item()))
        max_values = int(label.max())#np.amax(label, axis=(1, 2, 3, 4))
        y_true.append(max_values)
        line = str(ID + "    " + str(ret2.item()) + "    " + str(max_values))+ "    "+str(ret1.item())
